...
# Класс `GraphNode`
Базовая единица всего графа - узел. Узел хранит информацию о своей позиции в пространстве и ссылки на своих соселей `сверху`, `снизу`, `слева` и `справа`. Ссылки может быть и не быть, а вместо неё может быть `null`, если связь недопустима (см. пункт "Graph.createGraphFrom()").
