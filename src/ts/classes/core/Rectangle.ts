...
export default class RectangleShape extends PrimitiveShape implements Rectangle {
    ...
    constructor(params: Rectangle){
        ...
        this.ports = new PortsMap({
            ...
            A: new Port({ letter: 'A', parent: this, angle: 180, r: 3, x: this.position.x - this.style.margin,  y: this.position.y + (this.size.height / 2) }),
            B: new Port({ letter: 'B', parent: this, angle: 90, r: 3, x: this.position.x + (this.size.width / 2),  y: this.position.y - this.style.margin}),
            C: new Port({ letter: 'C', parent: this, angle: 0, r: 3, x: this.position.x + this.size.width + this.style.margin,  y: this.position.y + (this.size.height / 2) }),
            D: new Port({ letter: 'D', parent: this, angle: 270, r: 3, x: this.position.x + (this.size.width / 2),  y: this.position.y + this.size.height + this.style.margin}),
        })
    }
